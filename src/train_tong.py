...
import pypianoroll
import imageio
from data_proc import init_stage1_data, init_stage2_data, init_tong_stage1_data
...
DATA_STRINT = "tong_19_7_9_" # 9999的版本
DATA_STRING = "tong_19_7_11_1_"
...
def print_metrix(sess, data, params, config, string = None):
...
def main():
    ...
    train_x = init_tong_stage1_data(m_train_x)

    ###################### test eval
    # print_metrix(sess = sess, data=train_x, params=params, config=config)
    ...
    timesteps = int(train_x.shape[1])
    print("input dim", input_dim, "timesteps", timesteps)
    batch_size = 32
    latent_dim = 256
    stage1_vrae = vrae_muse(input_dim,
                     timesteps=timesteps,
    ...
    print("lets training!!!")
    spli = batch_size * ((len_track//batch_size) - 1)
    ...
    if TRAIN_STAGE1:
        ...
        x = train_x[0:spli]
        print("NO",np.shape(x))
        stage1_vrae.vae.fit(x, x, epochs=30, )
        stage1_vrae.vae.save_weights(DATA_STRING + "track_vae.h5")
        stage1_vrae.encoder.save_weights(DATA_STRING + "track_enc.h5")
        stage1_vrae.generator.save_weights(DATA_STRING + "track_gen.h5")

    else:
        print("loading track vae..")
        stage1_vrae.vae.load_weights(DATA_STRING+"track_vae.h5")
        stage1_vrae.encoder.load_weights(DATA_STRING+"track_enc.h5")
        stage1_vrae.generator.load_weights(DATA_STRING+"track_gen.h5")
    ...
    #     train_bns = []
    #     bn_refines = []
    #     flag = 1 ##1是以bar为单位 2是以列为单位
    #     flag_loss = 2## 1是MSE，2是bar的b cross, 3是列的b cross
    #
    ...
    #
    #             bn.model.fit(train_bn, x, batch_size = 512, epochs = 30)
    ...
    if GEN_STAGE1 == True:
        ...
        tests1 = []
        tests2 = []
        print("is m_train_x shape change ?", np.shape(m_train_x))
        train_x = init_stage1_data(m_train_x)
        print(np.shape(train_x), len_track)
        len_track = 5
        for i in range(len_track):
            test_x = train_x[i][0:32]
            pre = stage1_vrae.vae.predict(test_x) #直接
        ...
        # print_metrix(sess, pres_bns, params, config, "preee222")
